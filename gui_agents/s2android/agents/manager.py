...
logger = logging.getLogger("androidenv.manager")

NUM_IMAGE_TOKEN = 1105  # Value set of screen of size 1920x1080 for openai vision


class AndroidManager(BaseModule):
    """Android-specific Manager that decomposes tasks into high-level subgoals for Android UI automation"""
    ...
    def __init__(
        ...
        # Initialize the Android grounding agent
        self.grounding_agent = grounding_agent

        # Initialize the Android-specific planner with enhanced prompts
        sys_prompt = PROCEDURAL_MEMORY.COMBINED_MANAGER_PROMPT
        self.generator_agent = self._create_agent(sys_prompt)

        # Initialize the remaining modules with enhanced prompts
        ...
        # Enable knowledge base for Android with enhanced capabilities
    ...
    def _generate_android_step_by_step_plan(
        self,
        observation: Dict,
        instruction: str,
        ...
        """Generate Android-specific step-by-step plan with enhanced knowledge integration"""

        # Converts a list of DAG Nodes into a natural language list
        ...
        # Enhanced knowledge base integration for Android
        if self.turn_count == 0:
            ...
            logger.info(
                "SIMILAR ANDROID TASK EXPERIENCE: %s",
            ...
            if self.search_engine is not None:
                ...
                logger.info("RETRIEVED ANDROID KNOWLEDGE: %s", retrieved_knowledge)
                ...
                if retrieved_knowledge is not None:
                    ...
                    logger.info("INTEGRATED ANDROID KNOWLEDGE: %s", integrated_knowledge)

            integrated_knowledge = integrated_knowledge or retrieved_experience

            # Add the integrated knowledge to the task instruction
            if integrated_knowledge:
                instruction += f"\nYou may refer to some retrieved Android knowledge if you think they are useful.{integrated_knowledge}"
        ...
        # Enhanced device context with more detailed information
        device_info = observation.get('device_info', {})
        device_context = f"""
Device Context:
- Screen Size: {device_info.get('screen_size', 'Unknown')}
- Current Activity: {device_info.get('current_activity', 'Unknown')}
- UI Elements: {device_info.get('num_ui_elements', 0)} elements available
- Device: {device_info.get('device_model', 'Android Device')}
- Android Version: {device_info.get('android_version', 'Unknown')}
- Available Apps: {device_info.get('available_apps', 'Unknown')}
- System State: {device_info.get('system_state', 'Unknown')}
- Network Status: {device_info.get('network_status', 'Unknown')}
"""

        # Generate context-specific messages with enhanced planning
        if failed_subtask:
            generator_message = (
                f"The Android subtask {failed_subtask} cannot be completed. "
                f"Please generate a new plan for the remainder of the trajectory.\n\n"
                f"Successfully Completed Android Subtasks:\n{format_subtask_list(completed_subtasks_list)}\n"
                f"{device_context}\n"
                f"Consider alternative approaches and error recovery strategies for this Android task."
            )
        elif len(completed_subtasks_list) + len(remaining_subtasks_list) > 0:
            generator_message = (
                "The current Android trajectory and device state is provided. "
                f"Please revise the plan for the following trajectory.\n\n"
                f"Successfully Completed Android Subtasks:\n{format_subtask_list(completed_subtasks_list)}\n"
                f"Future Remaining Android Subtasks:\n{format_subtask_list(remaining_subtasks_list)}\n"
                f"{device_context}\n"
                f"Ensure the revised plan considers Android-specific constraints and UI patterns."
            )
        else:
            generator_message = (
                f"Please generate the initial Android plan for the task.\n{device_context}\n"
                f"Focus on Android-specific UI patterns, navigation flows, and error handling strategies."
            )

        logger.info("ENHANCED ANDROID GENERATOR MESSAGE: %s", generator_message)
        ...
        logger.info("GENERATING ENHANCED ANDROID HIGH LEVEL PLAN")

        plan = call_llm_safe(self.generator_agent)
        if plan == "":
            raise Exception("Enhanced Android Plan Generation Failed - Fix the Prompt")

        logger.info("ENHANCED ANDROID HIGH LEVEL STEP BY STEP PLAN: %s", plan)
    ...
    def _generate_android_dag(self, instruction: str, plan: str) -> Tuple[Dict, Dag]:
        """Generate Android-specific DAG from plan with enhanced translation"""
        # Reset for re-planning case
        ...
        self.dag_translator_agent.add_message(
            f"Android Instruction: {instruction}\nAndroid Plan: {plan}", role="user"
        )

        logger.info("GENERATING ENHANCED ANDROID DAG")

        # Generate DAG
        dag_raw = call_llm_safe(self.dag_translator_agent)
        dag = parse_dag(dag_raw)

        logger.info("Generated Enhanced Android DAG: %s", dag_raw)
    ...
    def get_action_queue(
        ...
        """Generate the Android action list based on the instruction with enhanced planning"""
        
        # Always show reasoning
        print(f"Manager plan")
        
        planner_info, plan = self._generate_android_step_by_step_plan(
            observation,
            instruction,
            failed_subtask,
        ...
        # Generate the Android DAG
        dag_info, dag = self._generate_android_dag(instruction, plan)

        # Topological sort of the DAG
        action_queue = self._topological_sort(dag)

        # Always show subtask breakdown
        print(f"   Subtasks: {[s.name for s in action_queue]}")
